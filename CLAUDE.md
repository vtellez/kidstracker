...
- **Sondeo casi en tiempo real**: `startPolling()` hace pull cada `POLL_MS` (5 s) solo con la pestaña visible; se autopausa tras `IDLE_STOP_MS` (90 s) sin interacción ni cambios, y se reactiva con cualquier toque/tecla o al volver el foco. Equilibrio entre inmediatez y no agotar la cuota gratis (10k req/mes); si se nota consumo, subir `POLL_MS` o bajar `IDLE_STOP_MS`. Pull también al abrir y al recuperar el foco.
